...
<body class="use-poppins-normal bg-cover bg-no-repeat bg-center" style="background-image: url('/assets/auth.png')">
  <div id="wrapper" class="flex min-h-screen">
    ...
    <main id="main-content" class="main-content flex-1 px-4 py-4 md:px-6 md:py-6 md:ml-[240px] transition-all duration-300 ease-in-out">
      ...
      <div class="mt-6 rounded-2xl shadow-lg bg-white px-4 md:px-6 py-6">
        {{-- Header --}}
        ...
        <div class="p-6 bg-white rounded-xl">
          ...
          <form action="{{ route('landboard.penalty.update') }}" method="POST" class="space-y-5">
            ...
            {{-- Aktifkan penalti --}}
            <label class="flex items-start gap-3 font-semibold text-gray-700 break-words whitespace-normal">
              <input type="checkbox" id="penaltyEnabled" name="is_penalty_enabled" {{ $landboard->is_penalty_enabled ? 'checked' : '' }} class="h-5 w-5 rounded mt-1">
              <span class="break-words whitespace-normal">Aktifkan Penalti</span>
            </label>
            ...
            {{-- Penalti keluar --}}
            <label class="flex items-start gap-3 text-gray-700 font-semibold break-words whitespace-normal">
              <input type="checkbox" id="moveoutEnabled" name="is_penalty_on_moveout" {{ $landboard->is_penalty_on_moveout ? 'checked' : '' }} class="h-5 w-5 rounded mt-1">
              <span class="break-words whitespace-normal">Penalti Jika Keluar Sebelum Masa Sewa Habis</span>
            </label>
            ...
            <label class="flex items-start gap-3 text-gray-700 font-semibold break-words whitespace-normal">
              <input type="checkbox" id="roomChangeEnabled" name="is_penalty_on_room_change" {{ $landboard->is_penalty_on_room_change ? 'checked' : '' }} class="h-5 w-5 rounded mt-1">
              <span class="break-words whitespace-normal">Penalti Jika Pindah Kamar</span>
            </label>
            ...
            {{-- Perpanjangan sewa --}}
            <div>
              <label for="decision_days_before_end" class="mb-1 block text-sm text-gray-700 break-words whitespace-normal">
